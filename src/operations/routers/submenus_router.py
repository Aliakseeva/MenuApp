...
submenu_router = APIRouter(prefix='/api/v1/menus/{menu_id}/submenus', tags=['submenus'])


@submenu_router.post('/', response_model=sm.Submenu, status_code=201)
def create_submenu(menu_id: int, submenu: sm.SubmenuCreate, db: Session):
...
def read_all_submenus(menu_id: int, db: Session):
...
def read_submenu(submenu_id: int, db: Session):
...
def update_submenu(submenu_id: int, submenu: sm.SubmenuUpdate, db: Session):
