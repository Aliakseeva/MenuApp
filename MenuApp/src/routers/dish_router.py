...
async def update_dish(
        ...
        dish_id: int, dish: d.DishCreateUpdate, db: AsyncSession = Depends(get_db),
) -> dict[str, int]:
    key = f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}'
    ...
    await update.update_dish(db=db, dish_id=dish_id,
                             new_title=dish.title, new_descr=dish.description, new_price=dish.price)
    return upd_dish
