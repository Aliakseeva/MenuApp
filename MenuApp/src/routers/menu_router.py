...
async def create_menu(menu: m.MenuCreateUpdate, db: AsyncSession = Depends(get_db)) -> dict[str, int]:
    key = '/api/v1/menus'
...
async def update_menu(
    menu_id: int, menu: m.MenuCreateUpdate, db: AsyncSession = Depends(get_db),
) -> dict[str, int]:
    key = f'/api/v1/menus/{menu_id}'
    ...
    await update.update_menu(db=db, menu_id=menu_id, new_title=menu.title, new_descr=menu.description)
    return upd_menu
...
async def delete_menu(menu_id: int, db: AsyncSession = Depends(get_db)) -> dict[str, bool]:
    key = '/api/v1/menus'
